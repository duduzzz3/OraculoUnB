...
def aba_comparativo(df: pd.DataFrame) -> None:
    ...
    with st.expander("Metodologia de similaridade e níveis de valor justo", expanded=False):
        st.markdown(
            ...
            |---|---:|---|
            | **Autor** | **30 pts** | Critério principal. Obras do mesmo autor tendem a apresentar maior comparabilidade mercadológica, pois compartilham trajetória, assinatura, reconhecimento e posicionamento no mercado. |
            | **Técnica** | **25 pts** | Compara o meio artístico e a materialidade da obra. Técnicas iguais aumentam fortemente a similaridade entre as obras. |
            | **Dimensões** | **20 pts** | Compara a escala física da obra pela área aproximada. Quanto mais próximas as dimensões, maior a pontuação. |
            | **Ano** | **15 pts** | Compara a proximidade temporal da produção, considerando fase do artista, contexto e período de mercado. |
            | **Preço** | **10 pts** | Critério complementar e de menor peso. Quando disponível, compara a ordem de grandeza dos preços; quando ausente, não impede que obras do mesmo autor, técnica, dimensão e período sejam reconhecidas como comparáveis. |
